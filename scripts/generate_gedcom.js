...
// Usage: node scripts/generate_gedcom.js [count] [outpath]
const count = parseInt(process.argv[2], 10) || 2000;
const outPath = process.argv[3] || path.join(__dirname, '..', 'data', `complex_${count}.ged`);
...
// We'll generate individuals and families. Strategy:
// - Create N individuals.
// - Pair adjacent individuals into families (husb-wife) for first 60% of people.
// - Assign children randomly to families to create multiple generations.
...
for (let i=1;i<=count;i++){
  ...
  const sex = i%2===0 ? 'F' : 'M';
  const given = `Given${i}`;
  const surname = `Surname${Math.ceil(i/20)}`; // repeat surnames to create clusters
  ...
  const deathDate = deathRoll > 0.85 ? randomDate(birthYear+20, Math.min(birthYear+100, 2025)) : null;
  const note = `Auto-generated person ${i}. This record is part of a large synthetic GEDCOM for testing purposes. Lines: ${Math.ceil(Math.random()*5)}.`;

  individuals.push({ id, name, sex, birthDate, deathDate, note });
}

// Families: pair adjacent individuals as spouses for first ~60%
...
// Assign children: remaining individuals (after spouses) become children of random families
const firstChildIndex = spouseCount*2 + 1;
for (let i=firstChildIndex; i<=count; i++){
  const childId = `@I${i}@`;
  // choose 1-3 families to belong to (some complexity)
  const pick = families[randInt(0, families.length-1)];
  if (pick) pick.children.push(childId);
}

// Also create some multi-generation links by adding some of the earlier individuals as children of later pairs
for (let k=0;k<Math.floor(families.length*0.1);k++){
  ...
  if (!fam.children.includes(childId) && Math.random()>0.7) fam.children.push(childId);
}
...
individuals.forEach(ind => {
  ...
  // Randomly reference family membership
  if (Math.random() > 0.5) stream.write(`1 FAMS @F${randInt(1,families.length)}@\n`);
  if (Math.random() > 0.8) stream.write(`1 FAMC @F${randInt(1,families.length)}@\n`);
