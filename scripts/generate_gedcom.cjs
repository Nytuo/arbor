...
// Usage: node scripts/generate_gedcom.cjs [count] [outpath]
...
for (let k=0;k<Math.floor(families.length*0.1);k++){
  ...
  if (fam && !fam.children.includes(childId) && Math.random()>0.7) fam.children.push(childId);
}
...
individuals.forEach(ind => {
  ...
  if (Math.random() > 0.5 && families.length>0) stream.write(`1 FAMS @F${randInt(1,families.length)}@\n`);
  if (Math.random() > 0.8 && families.length>0) stream.write(`1 FAMC @F${randInt(1,families.length)}@\n`);
