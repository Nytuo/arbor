  ...
  const filteredPeople = people.filter(
    (p) =>
      ...
      `${p.firstName} ${p.lastName}`
        .toLowerCase()
        .includes(relativeSearch.toLowerCase()),
  );
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <DatePicker
                  selected={
                    formData.deathDate ? new Date(formData.deathDate) : null
                  }
                  onChange={(date: Date | null) => {
                    const value = date ? format(date, "yyyy-MM-dd") : "";
          ...
          <div className="space-y-2">
            {personRelationships.length > 0 ? (
              personRelationships.map((r) => {
                ...
                const isChild =
                  r.type === "PARENT_CHILD" && r.fromId === person.id;
                const isParent =
                  r.type === "PARENT_CHILD" && r.toId === person.id;

                return (
                  <div
                    key={r.id}
                    className="flex items-center justify-between p-2.5 bg-slate-50 rounded-xl border border-slate-100 text-sm group transition-all hover:bg-white hover:shadow-sm"
                  >
                    <div className="flex items-center gap-3 overflow-hidden">
                      <div
                        className={`p-1.5 rounded-lg ${r.type === "SPOUSE" ? "bg-pink-100 text-pink-600" : "bg-blue-100 text-blue-600"}`}
                      >
                        {r.type === "SPOUSE" ? (
                          <Heart size={14} />
                        ) : (
                    ...
                    <button
                      ...
                      className="text-slate-300 hover:text-red-500 p-1 rounded-md opacity-0 group-hover:opacity-100 transition-all"
                    >
                      <X size={14} />
                    </button>
                  </div>
